...
public class ConnectTest {
	...
	{
		Connection conn=null; //초기값 없으니 null
		Statement stmt=null; //오라클의 sql문을 전송시켜줌
		ResultSet rs=null; //조회하려면 ResultSet 필요
		
		
		
		String sq1="select * from sawon order by num";
		
		try {
			conn=DriverManager.getConnection(URL, "stu", "a1234");
			
			stmt=conn.createStatement(); //sqㅣ문을 전달해줄수있는 메소드
			rs=stmt.executeQuery(sq1);
			
			//여러줄을 가져올때는 while문으로 가져온다
			//rs.next() : 다음데이타로 이동하면서 true반환, 더이상데이타 없으면 false반환
			while(rs.next())
			{
				//db로 부터 데이타 가져오기
	...
	public void connectShop()
	{
		...
		try {
			conn=DriverManager.getConnection(URL, "hyoyoung", "a1234");
	...
	public void sawonRead()
	{
		...
		try {
			...
			while(rs.next())
			{
				...
				System.out.println(buseo+"\t"+count+"\t"+pay);
