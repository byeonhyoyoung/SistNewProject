...
public class MemgaipDao {
	...
	DbConnect db=new DbConnect(); //데이터베이스 연결
	
	//아이디가 존재하면 true리턴
	//isIdCheck(String m_id)메서드는 특정아이디가 데이터베이스에 이미 존재하는지 여부 확인
	public boolean isIdCheck(String m_id)
	{
		boolean b=false; //있을경우에만 true로 변경
		
		Connection conn=db.getConnection();
		PreparedStatement pstmt=null; //SQL 쿼리 실행하여 id 존재여부 확인
		ResultSet rs=null;
...
String sql="select * from memgaip where m_id=?";
	...
	public void deleteMemgaip(String num)
	{
		...
		String sql="select from memgaip where num=?";
	...
	public void updateMemgaip(MemgaipDto dto)
	{
		...
		String sql="update memgaip set m_name=?,m_hp=?,m_photo=? where num=?";
